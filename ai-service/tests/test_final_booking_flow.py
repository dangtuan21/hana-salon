#!/usr/bin/env python3
"""
Test the final complete booking flow with proper phone formatting
"""
...
def test_final_booking_flow():
    ...
    print("🧪 Testing FINAL Complete Booking Flow...")
    print("=" * 60)
    
    # Test with a 10-digit phone number for better formatting
    print("📨 Step 1: Starting conversation with properly formatted phone...")
    
    start_response = requests.post(f"{base_url}/conversation/start", json={
        "message": "Hi, I'm Lisa Chen, phone 555-123-4567. I want Gel Manicure for Friday at 2pm"
    })
    ...
    if start_response.status_code == 200:
        ...
        if confirm_response.status_code == 200:
            ...
            if any('availability' in str(action).lower() for action in actions_taken):
            ...
            # 5. Booking Creation Attempted
            if any('booking' in str(action).lower() and 'created' in str(action).lower() for action in actions_taken):
                print(f"   ✅ 5. Booking creation attempted")
                success_count += 1
            else:
                print(f"   ❌ 5. Booking creation not attempted")
